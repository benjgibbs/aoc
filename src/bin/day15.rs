...
impl Cave {
    pub fn new(lines: &Vec<String>) -> Cave {
        ...
        Cave { risk: risk, max_x: max_x, max_y: lines.len() }
    }
    ...
    pub fn neighbours(&self, i: usize) -> Vec<usize> {
        ...
        adjacents.iter().map(|a| (pos.0 as i32 + a.0, pos.1 as i32 + a.1))
            .filter(|p|p.0 >= 0 && p.1 >= 0 && (p.0 as usize) < self.max_x && (p.1 as usize) < self.max_y)
    ...
    fn extend(&self, by: usize) -> Cave {
        ...
        for y in 0..self.max_y*by {
            for x in 0..self.max_x*by {
                let mut risk = 0;
                if x < self.max_x && y < self.max_x {
                    risk = self.risk_by_pos((x, y));
                } else if y < self.max_y { // x >= max
                    let xpos= x - self.max_x;
                    let ypos =y;
                    risk = risks[xpos + ypos * self.max_x * by] + 1;
                } else  {
                    let xpos= x;
        ...
        Cave{risk: risks, max_x: self.max_x * by, max_y: self.max_y *by }
    }
}
...
fn search(cave: Cave) -> Option<u32> {
    ...
    while let Some(state) = heap.pop() {
        if state.pos == end { return Some(state.risk) }
        ...
        for n in cave.neighbours(state.pos) {
            let next = State{risk: state.risk + cave.risk(n), pos: n};
...
fn part2(lines: &Vec<String>) -> Option<u32> {
    ...
    for y in 0..cave.max_y {
        for x in 0..cave.max_x {
            print!("{}", cave.risk_by_pos((x,y)));
        }
        println!();
    }
